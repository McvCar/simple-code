...
module.exports = {
	...
	onLoad(){
		...
		// 首次使用拷贝模板到可写路径
		if(!tools.isDirectory(USER_TEMPLE_PATH) && tools.isDirectory(TEMPLE_PATH)){
			tools.createDir(USER_TEMPLE_PATH)
			let fileList = tools.getFileList(TEMPLE_PATH,[]);
			for (let i = 0; i < fileList.length; i++) {
				const filePath = fileList[i];
				tools.copyFile(filePath,path.join(USER_TEMPLE_PATH,path.basename(filePath)))
			}
		}
	...
	upTempletList(){
		...
		for (let i = 0; i < fileList.length; i++) {
			const filePath = fileList[i];
			if(filePath.indexOf('.DS_Store') != -1 || filePath.indexOf('desktop.ini') != -1){
				continue;
			}
	...
	newFileAndBindNode(templePath){
		...
		Editor.Scene.callSceneScript('simple-code', 'get-curr-scene-url-and-node',{}, function (err, args) {
			if(args == null) {
				return;
			}

			try {
				let saveUrl =  require(USER_NEW_FILE_RULE).getSavePath(templePath,args.url,args.currNodeName);
				...
				Editor.error(tools.translateZhAndEn('检测新建脚本规则是否填错:','Check if new script rule is filled incorrectly:'),error)
			}
		});
	...
	updateMenu(){
		...
		if(selectInfo == null || selectInfo.id == null){
			// 清除菜单
			Editor.Ipc.sendToMain('simple-code:setMenuConfig',{id:"cc-new-file",menuCfg:undefined})
			return;
		}
		...
		submenu.push({ label: tools.translateZhAndEn("刷新模板",'Refresh Templates'), enabled: true, cmd: "refresh-template"});
		submenu.push({ label: tools.translateZhAndEn("自定义模板",'Custom Template'), enabled: true, cmd: "custom-template"});
		submenu.push({ label: tools.translateZhAndEn("自定义生成规则",'Custom Build Rules'), enabled: true, cmd: "custom-build-templet-rules"});


		let menuCfg = {
			layerMenu : [
				{ type: 'separator' },
				{ label : tools.translate('new-script-bind'), enabled:true, submenu:submenu, }, // 生成拖拽组件
			],
			assetMenu : [
				{ type: 'separator' },
				{ label : tools.translate('new-script-templet'), enabled:true, submenu:submenu, }, // 生成拖拽组件
			],
		}
		this.menuCfg = menuCfg;
		Editor.Ipc.sendToMain('simple-code:setMenuConfig',{id:"cc-new-file",menuCfg:menuCfg})
	},
	...
	messages:{
		...
		'new-script-templet'(e,args){
			...
			if(selectInfo.type == 'asset'){
				...
				let filePath = Editor.remote.assetdb.uuidToFspath(selectInfo.id)
				
				let fspath = tools.isDirectory(filePath) ? filePath : path.dirname(filePath);
				if(!templePath || !tools.isDirectory(fspath)){
					return;
				}

				let findObj = fspath.match(config.prsPath) ;
				if(!findObj || !fspath.substr(findObj.index).match('assets')){
					alert(tools.translateZhAndEn('不能选择在根目录创建模板','Cannot choose to create template in root directory'))
					return;
				}
