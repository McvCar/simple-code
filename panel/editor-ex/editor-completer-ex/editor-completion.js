...
class ImportCompletion
{
    ...
    getPathItems(model,position,text,suggestions){
        ...
        let importFsPath = tools.relativePathTofsPath(item.path.substr(0,i+1),model.fsPath);
        if(!fileTools.isDirectory(importFsPath)){
            return;// 没有目录
        }
    ...
    getDirItems(fsPath,suggestions,useExtnames,isShowExtname){
        ...
        files.forEach((dirFile, index) => 
        {
            ...
            let extname = fileTools.getFileExtname(dirFile);
            if(!isShowExtname){
                dirFile = fileTools.getFileName(dirFile);
            }
            suggestions.push({
                label: dirFile,
                insertText: dirFile,
